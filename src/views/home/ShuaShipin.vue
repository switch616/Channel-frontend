  ...
  <div class="shuashipin-container">
    <div class="video-swiper" ref="swiperRef" @wheel.passive="onWheel">
      <div
        ...
        class="video-item"
        :data-type="slot.type"
        :class="{ active: slot.type === 'current', prev: slot.type === 'prev', next: slot.type === 'next' }"
      >
        <!-- 背景模糊 -->
        <div v-if="slot.video" class="video-bg-blur" :style="{ backgroundImage: `url('${slot.video.cover_image}')` }"></div>
        ...
        <div class="video-player-wrapper">
          <video
            ...
            :src="slot.video.videoUrl"
            :poster="slot.video.cover_image"
            class="video-player"
            autoplay
            muted
            playsinline
...
function syncSlots(): void {
  ...
  nextTick(() => onCanPlay()) // 确保 current 播放
}
...
function handleWaiting(): void {
  ...
  loadingTimeout = window.setTimeout(() => (loading.value = true), 150)
}
