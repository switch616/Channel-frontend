...
export function decryptData(encryptedData: string): any | null {
  try {
    ...
    console.error('解密错误:', (error as Error).message)
...
// 设置 token（支持记住密码）
export function setToken(token: string, expiresInSeconds: number = 3600, remember: boolean = false): void {
  const expiresAt = Date.now() + expiresInSeconds * 1000
  const payload: TokenData = { token, expiresAt }
